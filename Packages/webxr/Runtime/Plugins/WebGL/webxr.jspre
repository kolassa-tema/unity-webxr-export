...
`;
const blitShaderFragment = `#version 300 es
...
function compileShader(gl, source, type) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error(
        `An error occurred compiling the ${type === gl.VERTEX_SHADER ? 'vertex' : 'fragment'} shader:`,
...
void main()
{
	...
	(function () {
	  ...
	  XRManager.prototype.attachEventListeners = function () {
		...
		var onSetCameraTexture = this.SetCameraTexture.bind(this);
    ...
    function outputImageAsBase64(gl,fb,width,height) {
	  ...
	  XRManager.prototype.SetCameraTexture = function(textureId) {
        console.log("Setting camera texture: "+textureId);
        const canvas = document.querySelector("canvas");
        if (!canvas) {
          console.error("Canvas element not found.");
          return;
        ...
        // Get the WebGL rendering context from the canvas
        const gl = canvas.getContext("webgl2", { xrCompatible: true });
        this.blitShader = createShaderProgram(gl, blitShaderVertex, blitShaderFragment)
        console.log("Shader program created: "+this.blitShader);
        this.cameraTexture = GL.textures[textureId];
	  }
